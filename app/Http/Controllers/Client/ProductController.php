...
class ProductController extends Controller
{
    ...
    {
        $data['list'] = Product::where(['status'=>1])->orderBy('id','DESC')->get(['id','category','name','discount','price','images','slug','cate_slug','type_slug']);
    ...
    public function detail_product($cate,$slug)
    {
        ...
        $data['productlq'] = Product::where(['cate_slug'=>$cate, 'status'=>1])->get(['id','name','images','discount','price','slug','cate_slug','type_slug','description','category']);
...
// return;
    ...
    public function getProduct(Request $request)
    {
        ...
        ->first();
        $html = view('layouts.product.quick-view', $data)->render();
        return response()->json([
            'status' => 200,
            'html' => $html,
        ]);
    ...
    public function getProductForContact(Request $request)
    {
        $id = $request->id;
        $data['product'] = Product::where([
            'id'=> $id
        ...
        ->first(['id', 'name']);
        $html = view('layouts.product.contact-form', $data)->render();
        return response()->json([
            'status' => 200,
            'html' => $html,
        ]);
